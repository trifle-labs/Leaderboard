...
describe('Leaderboard Robust Scaling Tests', function () {
  ...
  // Helper class to track expected leaderboard state in JavaScript
    ...
    it('should handle hundreds of random inserts and removals correctly', async function () {
      ...
      for (let i = 0; i < NUM_OPERATIONS; i++) {
        ...
        if (operationType === 'insert') {
          ...
          await leaderboard.insert(value, wallet.address);
          ...
          await leaderboard.remove(addressToRemove);
        ...
        if ((i + 1) % 100 === 0 || i === NUM_OPERATIONS - 1) {
          ...
          const treeSize = await leaderboard.size();
          ...
          if (tracker.size > 0) {
            ...
            for (let j = 0; j < tracker.size; j += step) {
              const treeValue = await leaderboard.getValueAtIndex(j);
              const treeOwner = await leaderboard.getOwnerAtIndex(j);
  ...
  describe('Successive Value Insertions', function () {
    it('should handle hundreds of successively increasing insertions', async function () {
      ...
      async function validateIndex(index) {
        const treeValue = await leaderboard.getValueAtIndex(index);
        const treeOwner = await leaderboard.getOwnerAtIndex(index);
      ...
      async function validateCompleteStructure() {
        ...
        for (let i = 0; i < tracker.size; i++) {
          const treeValue = await leaderboard.getValueAtIndex(i);
          const trackerEntry = tracker.getByIndex(i);
          expect(treeValue).to.equal(trackerEntry.value);

          const treeOwner = await leaderboard.getOwnerAtIndex(i);
          expect(treeOwner).to.equal(trackerEntry.address);
        }
      }
    });
          ...
          if (tracker.size > 0) {
            ...
            for (const idx of indicesToCheck) {
              if (idx >= 0 && idx < tracker.size) {
                const treeValue = await leaderboard.getValueAtIndex(idx);
  ...
  describe('Same Value Insertions', function () {
    it('should handle hundreds of same-value insertions with correct nonce ordering', async function () {
      ...
      for (let i = 0; i < NUM_OPERATIONS; i++) {
        ...
        // Insert same value
        await leaderboard.insert(SAME_VALUE, wallet.address);
        ...
        if ((i + 1) % 50 === 0 || i === NUM_OPERATIONS - 1) {
          ...
          if (i >= 2) {
            ...
            const newest = await leaderboard.getOwnerAtIndex(0);
            const middle = await leaderboard.getOwnerAtIndex(Math.floor(i / 2));
            const oldest = await leaderboard.getOwnerAtIndex(i);
            ...
            const newestNonce = await leaderboard.getNonce(newest);
            const oldestNonce = await leaderboard.getNonce(oldest);
      ...
      for (let i = 0; i < tracker.size - 1; i++) {
        const current = await leaderboard.getOwnerAtIndex(i);
        const next = await leaderboard.getOwnerAtIndex(i + 1);

        const currentNonce = await leaderboard.getNonce(current);
        const nextNonce = await leaderboard.getNonce(next);
  ...
  describe('Specific Sequence Tests', function () {
    it('should maintain correct structure after specific insert/remove sequences', async function () {
      ...
      // First, create a leaderboard with 100 nodes in a specific pattern
      ...
      for (let i = 0; i < NUM_INITIAL; i++) {
        ...
        await leaderboard.insert(value, wallets[i].address);
        tracker.addEntry(
          wallets[i].address,
          value,
          (await ethers.provider.getBlock('latest')).timestamp
      ...
      // Now perform 50 removals and 50 insertions to stress the leaderboard structure
      ...
      // Check leaderboard is valid (using leaderboard's validation function)
      ...
      for (let i = 0; i < addresses.length; i++) {
        ...
        const treeIndex = await leaderboard.getIndexOfOwner(address);
